...
.controller('appController', ["$scope", "$http", "$window", "$parse", "$sce", "$cookieStore", "$filter","$location",function($scope, $http, $window,$parse,$sce, $cookieStore, $filter,$location) {
	...
	$scope.roomadd= function(details) {
		console.log(details);
		$scope.hotelsearch.rooms = $scope.hotelsearch.rooms+1;
		
	}
	$scope.roomremove= function(details) {
		console.log(details);
		$scope.hotelsearch.rooms = $scope.hotelsearch.rooms-1;
	...
	$scope.canceldt= function(details) {
		console.log(details);
		$scope.canceldts=details;
...
//################ Romita Saha ########################
	...
	$scope.searchHotalDetails = function(details) {
		console.log(details);
		$scope.inputjson={"userTrackId":$scope.userTrackId,"ResultIndex":details.ResultIndex,"Searchid":details.Searchid,"HotelId":details.HotelId,"Providerid":details.Providerid,"adminmarkup":details.adminmarkup,"usermarkup":details.usermarkup,"commission":details.commission};
		...
		$scope.loader = true;
			$http.post('searchHotalDetails',$scope.inputjson).success(function(data) {	
				...
				console.log($scope.hoteldetailsresult);
