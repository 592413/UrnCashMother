...
//	$scope.popremark= {};
    ...
    $scope.callDevice= function(details) {
	 ...
	 $scope.remDetails2={};
	 $scope.checkuserOPEN = function(details){
 		$scope.loader = true;
		...
		//$http.post('checkuserOPEN', details).success(function(data) {
 		$http.post('checkUserRazorPay', details).success(function(data) {
			$scope.remitter.mobile=details.mobile;
			console.log(data);
			$scope.loader = false; 
			if(data.status == "1"){
			    showDiv('div1');
			...
			/*	showDiv("div1");
				sshowDiv('sdiv2');*/
				$scope.remitterDetails=data.RemitterDetail;
				$scope.remitterbeneficiarylist=data.RemitterDetail.beneficiaryList;
		...
		$scope.RemitterRegisterOPEN =function(details){
    		$scope.loader = true;
    		...
    		//$http.post('RemitterRegisterOPEN', details).success(function(data) {
			$http.post('RemitterRegisterRazorPay', details).success(function(data) {
    			console.log(data);
    			$scope.loader = false; 
    			if(data.status == "1"){
    				$scope.showAlert("success", data.message);
	 ...
	 $scope.ValidateBeneficiaryEncore = function(details){
	 		...
	 		$scope.loader = true;
	 		console.log(details);
	 		$http.post('checkBeneficiaryEncore',details).success(function(data) {
	 			console.log(data);
	 			$scope.loader = false; 
	 			if(data.status == "1"){
	 				$scope.showAlert("success",data.message);
	 			//	$scope.checkuserOPEN(data);
	 ...
	 $scope.otpverifyOPEN = function(details){
	 		$scope.loader = true;
	 		details.mobile=$scope.remitter.mobile;
	 		console.log(details);
	 		//$http.post('otpverifyOPEN',details).success(function(data) {
	 			$http.post('otpverifyRazorpay',details).success(function(data) {
	 			console.log(data);
	 			$scope.loader = false; 
	 			if(data.status == "1"){
	 				$scope.showAlert("success", data.message);
	 ...
	 $scope.MoneytransferOPEN = function(details){
		 details.id=details.id.toString();
 		console.log(details);
 		...
 		if(details.sendType === undefined || details.sendType === null || details.sendType === "" ){
 			$scope.loader=false;	
 	...
 	//	$http.post('MoneytransferOPEN',details).success(function(data) {
 		
 		$http.post('MoneytransferRazorPay',details).success(function(data) {
 		$scope.loader = false;
 				if(data.status == "1"){
 					$scope.remitterDetails.Limit=data.Limit;
	    				...
	    				$scope.userDetails.balance=data.closingBalance;
	    				$scope.dmrInvoice=data.imptrans;
	    				//showDiv('div03');
 				}else{
 					$scope.showAlert("danger", data.message);
 					showDiv('div1');
	 ...
	 $scope.MoneytransferPAYTM = function(details){
		 details.id=details.id.toString();
 		console.log(details);
 		...
 		$http.post('MoneytransferPAYTM',details).success(function(data) {
 			$scope.loader = false;
 				if(data.status == "1"){
 					$scope.remitterDetails.Limit=data.Limit;
	    				$scope.showAlert("success", data.message);
	 ...
	 $scope.invoicback = function(){
		 showDiv('div1');
			sshowDiv('sdiv2'); 
	 }
	 
	 $scope.deletebeneOPEN = function(details,remmobile){
 		$scope.loader = true;
 		 details.id=details.id.toString();
 		console.log(details);
 		$http.post('deleteBeneficiaryRazorpay',details).success(function(data) {
 			console.log(data) 
 			$scope.loader = false;
 			if(data.status == "1"){
				$scope.showAlert("success",data.message);
	 ...
	 $scope.printdmrInvoice= function(details){
		 
		 $scope.dmrInvoice=details;
		 showDiv('div03');
	 }
	 ...
	 $scope.BeneficiaryRegistrationOPEN = function(details){
 		$scope.loader = true;
 		console.log(details);
	...
	//	console.log(format.test(details.bene_name));
		...
		if(format.test(details.beneMobileNumber)){
 		...
 		//$http.post('BeneficiaryRegistrationOPEN', details).success(function(data) {
 		$http.post('BeneficiaryRegistrationRazorPay', details).success(function(data) {	
 		console.log(data);
 			$scope.loader = false; 
 			if(data.status == "1"){
 				$scope.showAlert("success", data.message);
