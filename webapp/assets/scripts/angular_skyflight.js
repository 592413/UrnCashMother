...
/*	
    ...
    $scope.mydist ={};
 		...
 		$http.post('searchFlight',detail).success(function(data){
 			...
 			if(data.status != 0){
 				...
 				showDiv('div1');
	    			...
	    			// ********************* by Romita Saha *************************
					// #################### for One Way Price Range Slider ##################
	    			$scope.flightsRevAmp = data.report;
	    			...
	    			if (!flightsearchFlag) {
	    				$scope.fetchcalenderfare($scope.flight);
		    			flightsearchFlag = true;
					}
	    			
		 			}else{
		 				if(data.isDomestic==true ){
		 					showDiv('div2');
						...
						// ********************* by Romita Saha *************************
						// #################### for round trip  price slider ##################
		    			$scope.flightsRevAmp = data.report;
		    			
		    			$scope.separateSearchReport();
		    			
		    			$scope.getUpFilters();
		    		    $scope.getDownFilters();
						...
						for(var i=0;i<data.report.length;i++){
		    				...
		    				if (flags[$scope.flights[i].stops]) continue;
		                     ...
		                     //------------------------------
		                     //-----flight name collect-------------
		                     console.log($scope.flights[i].AirlineName);
		                     if (flags[$scope.flights[i].AirlineName]) continue;
		                     flags[$scope.flights[i].AirlineName] = true;
		                     nameoutput.push($scope.flights[i].AirlineName);
		                     //---------------------------------
	 ...
	 //Adult Count 
	 $scope.checkAdultCountPlus = function(adultCount) {
		
		if(adultCount >= 9){
			$('#adultPlas').prop('disabled', true);
			$('#adultMinus').prop('disabled', false);
		}else{
			$('#adultPlas').prop('disabled', false);
		}
	}
	 
	 $scope.checkAdultCountMinus = function(adultCount) {			
			if(adultCount < 9){
				$('#adultPlas').prop('disabled', false);
			}
		}
	 ...
	 //for child
	 $scope.checkChildCountPlus = function(childCount){
		 if(childCount >= 8){
			 $('.child-right-plus').prop('disabled', true);
				$('.child-left-minus').prop('disabled', false);
		 }else{
				$('.child-right-plus').prop('disabled', false);
			}
	 }
	 
	 $scope.checkChildCountMinus = function(childCount){
		 if(childCount < 8){
			 $('.child-right-plus').prop('disabled', false);
		 }
	 }
	...
	//for child + Adult
	 $scope.checkTotalAdultChildCount = function(adultCount, childCount){
		 
		 if(adultCount + childCount >= 9){
			 $('.child-right-plus').prop('disabled', true);
			 $('#adultPlas').prop('disabled', true);
		 }else if(adultCount + childCount < 9){
			 $('.child-right-plus').prop('disabled', false);
			 $('#adultPlas').prop('disabled', false);
		 }
	 }
	 ...
	 //for Infant
	 $scope.checkInfantCountPlus = function(infantCount) {}
	 
	 
	 $scope.separateSearchReport = function() {
		 $scope.upList = [];
		 $scope.downList = [];
		 ...
		 $scope.downMinFairVal = "";
		 $scope.downMaxFairVal = "";
		 
		 if( $scope.flightsRevAmp.length > 0){
			 for (var i = 0; i < $scope.flightsRevAmp.length; i++) {
				 if($scope.flightsRevAmp[i].FareBreakdown[0].TripIndicator==='1'){
					 
					 $scope.upList.push($scope.flightsRevAmp[i]); 
					 
				 }else if($scope.flightsRevAmp[i].FareBreakdown[0].TripIndicator==='2'){
					 
					 $scope.downList.push($scope.flightsRevAmp[i]); 
					 
				 }
			}
			 
		 $scope.cpyUpList = $scope.upList;
		 $scope.cpyDownList = $scope.downList;
		 
		 console.log('##############uplist###############');
		 console.log($scope.upList);
		 console.log('##############downlist###############');
		 console.log($scope.downList);
			 
		 for (var i = 0; i < $scope.upList.length; i++) {
				
			 $scope.upFairList[i] = $scope.upList[i].Fare.TotalAmount;
			 
			}
			
		 for (var i = 0; i < $scope.downList.length; i++) {
					
				 $scope.downFairList[i] = $scope.downList[i].Fare.TotalAmount;
				 
			}
		 console.log('##############upFairList###############');
		 console.log($scope.upFairList);
		 console.log('##############downFairList###############');
		 console.log($scope.downFairList);
		 
		 $scope.upMinFairVal = Math.min.apply(Math,$scope.upFairList);
		 $scope.upMaxFairVal = Math.max.apply(Math,$scope.upFairList);
		 
		 $scope.downMinFairVal = Math.min.apply(Math,$scope.downFairList);
		 $scope.downMaxFairVal = Math.max.apply(Math,$scope.downFairList);
		 
		 var aa = $scope.upMinFairVal;
		 var bb = $scope.upMaxFairVal;
		 var cc = $scope.downMinFairVal;
		 var dd = $scope.downMaxFairVal;
		 
		 console.log('##############upMinFairVal###############' + aa);
		 console.log('##############upMaxFairVal###############'+ bb);
		 console.log('##############downMinFairVal###############'+ cc);
		 console.log('##############downMaxFairVal###############'+ dd);
	...
	//by Romita *************************************************************
	$scope.getUpFilters = function(){
		 if($scope.flights.length>0){
				
			//Range slider config
			    $scope.roundtrip_slider_translate = {
			            minValue: $scope.upMinFairVal,
			            maxValue: $scope.upMaxFairVal,
			            options: {
			                ceil: $scope.upMaxFairVal,
			                floor: $scope.upMinFairVal,
			                translate: function (value) {
			                    return 'Rs. ' + value;
			                },
			                onEnd: function () {
			                	 $scope.roundTripupPiceRangeFilter($scope.roundtrip_slider_translate.minValue, $scope.roundtrip_slider_translate.maxValue);
				            }
			            }
			        };
			    console.log("MinVal:::: "+$scope.roundtrip_slider_translate.minValue+":::: MaxVal::::"+$scope.roundtrip_slider_translate.maxValue);
		 }
	...
	$scope.roundTripupPiceRangeFilter = function (minPrice, maxPrice){
		 console.log("Min ::: " +minPrice + "Max :: "+ maxPrice);
		 $scope.upSortedFlights = [];
		 
		 $scope.upList = [];
		 if($scope.cpyUpList.length>0){
			 for (var i = 0; i < $scope.cpyUpList.length; i++) {
				if ($scope.cpyUpList[i].Fare.TotalAmount >= minPrice && $scope.cpyUpList[i].Fare.TotalAmount <= maxPrice) {
					$scope.upSortedFlights.push($scope.cpyUpList[i]);
					//console.log("DDEEDDEEDDEE :::::: ");
					//console.log($scope.flights[i]);
				}
			}
			 //console.log("FFGGFFGGFFGG :::::: ");
			 console.log($scope.upSortedFlights);
			$scope.upList = $scope.upSortedFlights;
		 }
	 }
	...
	$scope.getDownFilters= function(){
		 if($scope.flights.length>0){
				
				//Range slider config
				    $scope.roundtrip_down_slider_translate = {
				            minValue: $scope.downMinFairVal,
				            maxValue: $scope.downMaxFairVal,
				            options: {
				                ceil: $scope.downMaxFairVal,
				                floor: $scope.downMinFairVal,
				                translate: function (value) {
				                    return 'Rs. ' + value;
				                },
				                onEnd: function () {
				                	 $scope.roundTripdownPiceRangeFilter($scope.roundtrip_down_slider_translate.minValue, $scope.roundtrip_down_slider_translate.maxValue);
					            }
				            }
				        };
				    console.log("MinVal:::: "+$scope.roundtrip_down_slider_translate.minValue+":::: MaxVal::::"+$scope.roundtrip_down_slider_translate.maxValue);
			 }
			 //console.log("$$$$$$$$$$$$$ ::::"+$scope.price_min);
			// console.log("$$$$$$$$$$$$$ ::::"+$scope.price_max);
		 }
	
	$scope.roundTripdownPiceRangeFilter = function (minPrice, maxPrice){
		 console.log("Min ::: " +minPrice + "Max :: "+ maxPrice);
		 $scope.downSortedFlights = [];
		 
		 $scope.downList = [];
		 if($scope.cpyDownList.length>0){
			 for (var i = 0; i < $scope.cpyDownList.length; i++) {
				if ($scope.cpyDownList[i].Fare.TotalAmount >= minPrice && $scope.cpyDownList[i].Fare.TotalAmount <= maxPrice) {
					$scope.downSortedFlights.push($scope.cpyDownList[i]);
					//console.log("DDEEDDEEDDEE :::::: ");
					//console.log($scope.flights[i]);
				}
			}
			 //console.log("FFGGFFGGFFGG :::::: ");
			 console.log($scope.downSortedFlights);
			$scope.downList = $scope.downSortedFlights;
		 }
	 }
	
	 
	//$scope.getDownFilters();
	//$scope.price_min = 1200;	 
	// $scope.price_max = 10000;
	 ...
	 $scope.getFilters = function(){
		 if($scope.flights.length>0){
			 $scope.price_min = $scope.min;
				
			 $scope.price_max = $scope.max;
			//Range slider config
			    $scope.slider_translate = {
			            minValue: $scope.price_min,
			            maxValue: $scope.price_max,
			            options: {
			                ceil: $scope.price_max,
			                floor: $scope.price_min,
			                translate: function (value) {
			                    return 'Rs. ' + value;
			                },
			                onEnd: function () {
			                	 $scope.priceRangeFilter($scope.slider_translate.minValue, $scope.slider_translate.maxValue);
				            }
			            }
			        };
			    console.log("MinVal:::: "+$scope.slider_translate.minValue+":::: MaxVal::::"+$scope.slider_translate.maxValue);
		 }
		 //console.log("$$$$$$$$$$$$$ ::::"+$scope.price_min);
		// console.log("$$$$$$$$$$$$$ ::::"+$scope.price_max);
	 }
	 ...
	 $scope.priceRangeFilter = function (minPrice, maxPrice){
		 console.log("Min ::: " +minPrice + "Max :: "+ maxPrice);
		 $scope.sortedFlights = [];
		 $scope.flights = [];
		 $scope.flights = $scope.flightsRevAmp;
		 if($scope.flightsRevAmp.length>0){
			 for (var i = 0; i < $scope.flightsRevAmp.length; i++) {
				if ($scope.flights[i].Fare.TotalAmount >= minPrice && $scope.flights[i].Fare.TotalAmount <= maxPrice) {
					$scope.sortedFlights.push($scope.flights[i]);
					//console.log("DDEEDDEEDDEE :::::: ");
					//console.log($scope.flights[i]);
				}
			}
			 //console.log("FFGGFFGGFFGG :::::: ");
			 console.log($scope.sortedFlights);
			$scope.flights = $scope.sortedFlights;
		 }
	 }
 					 ...
 					 if(data.status == "1"){
 						 ...
 						 $scope.sourceDetailfsc = data.requestDetails.requestDetails.source.split(',');
	    					 ...
	    					 if(data.requestDetails.adult!= null || data.requestDetails.adult!='' || data.requestDetails.adult!= undefined ){
	 	 					...
	 	 					$scope.showAlert("danger", data.Errormessage);
		 				 }
	 ...
	 $scope.getTaxround = function(detail,details2,request){
        ...
        $scope.selectSeat= function(detail){
		 ...
		 $scope.selectedseats = [];
	        ...
	        $scope.bookingFlightmodal = function(details){
	        	$scope.tatalflt=details;
	        }
    	...
    	$scope.infantPassport = [];
	 	 ...
	 	 $scope.bookingFlight = function(details){
	 		...
	 		details.isDomestic=$scope.result.isDomestic;
	 		if($scope.result.adultcount!= null || $scope.result.adultcount!='' || $scope.result.adultcount!= undefined ){
				for(var i = 1 ; i<=parseInt($scope.result.adultcount);i++){
	    				...
	    				if(adultdob === undefined || adultdob === null ||adultdob === ""){
	    	 				$scope.loader=false;
	    	 				$scope.showAlert("danger", "Enter adult dob");
	    	 				return false;
	    	 			}
	    				if($scope.result.isDomestic===false){
	    					var adultpassport = document.getElementById('adultpassport'+i).value.trim();
		    				if(adultpassport === undefined || adultpassport === null ||adultpassport === ""){
		    	 				$scope.loader=false;
		    	 				$scope.showAlert("danger", "Enter adult Passport NO");
		    	 				return false;
		    	 			}
		    				$scope.adultPassport.push(adultpassport);
	    				}
					...
					$scope.adultdetail = {
	    					...
	    					adultPassport:$scope.adultPassport
					}
			 }
	 		...
	 		if($scope.result.childcount!= null || $scope.result.childcount!='' || $scope.result.childcount!= undefined ){
				for(var i = 1 ; i<=parseInt($scope.result.childcount);i++){
    				...
    				if(childdob === undefined || childdob === null ||childdob === ""){
    	 				$scope.loader=false;
    	 				$scope.showAlert("danger", "Enter child dob");
    	 				return false;
    	 			}
    				if($scope.result.isDomestic===false){
    					var childpassport = document.getElementById('childpassport'+i).value.trim();
	    				if(childpassport === undefined || childpassport === null ||childpassport === ""){
	    	 				$scope.loader=false;
	    	 				$scope.showAlert("danger", "Enter Child Passport NO");
	    	 				return false;
	    	 			}
	    				$scope.childPassport.push(childpassport);
    				}
				...
				$scope.childdetail = {
            			...
            			childPassport:$scope.childPassport
					}
			 }
	 		...
	 		if($scope.result.infantcount!= null || $scope.result.infantcount!='' || $scope.result.infantcount!= undefined ){
				for(var i = 1 ; i<=parseInt($scope.result.infantcount);i++){
	    				...
	    				if(infantdob === undefined || infantdob === null ||infantdob === ""){
	    	 				$scope.loader=false;
	    	 				$scope.showAlert("danger", "Enter infant dob");
	    	 				return false;
	    	 			}
	    				if($scope.result.isDomestic===false){
	    					var infantpassport = document.getElementById('infantpassport'+i).value.trim();
		    				if(infantpassport === undefined || infantpassport === null ||infantpassport === ""){
		    	 				$scope.loader=false;
		    	 				$scope.showAlert("danger", "Enter Infant Passport NO");
		    	 				return false;
		    	 			}
		    				$scope.infantPassport.push(infantpassport);
	    				}
				...
				$scope.infantdetail = {
               			...
               			"infantPassport" : $scope.infantPassport
					}
			 }
	 		...
	 		$http.post('bookingFlight',$scope.inputJson).success(function(data){
	 			...
	 			$scope.loader=false;
	 			 if(data.status=="1"){
		 	    	...
		 	    	$scope.childLbag2=[];
		 	    	$scope.infantDOB=[];
		 	    	$scope.ticketdt=data.bookingresult.TicketDetails;
		 	    	$scope.adminmarkup=data.adminmarkup;
		 	    	$scope.usermarkup=data.usermarkup;
		 	    	showDiv('div1012');
...
$scope.showinternationalFlightDetailTabs = function(id) {
	var div = "#tabDetailinter" + id;
	$('div[id^=tabDetailinter]').hide();
	$(div).toggle();
}
