...
@Repository("apiParametersdao")
public class ApiParametersDaoImpl implements ApiParametersDao {
	...
	public List<ApiParameters> getAllApiParameters() {
		List<ApiParameters> list = new ArrayList<ApiParameters>();
		Session session = sessionFactory.openSession();
		try {
			list = session.createCriteria(ApiParameters.class).list();
			String message = "Method : getAllApi() ";
	...
	public ApiParameters getApiParametersByApId(String apiId) {
		...
		try {
			api = session.get(ApiParameters.class, apiId);
			String message = "Method : getApiByApId(apiId) || userId: " + api.getApiId();
			logger_log.warn(message);
	...
	public boolean insertApiParameters(ApiParameters api) {
		...
		try {
			session.save(api);
			String message = "QUERY : Save Api|| ID: " + api.getApiId();
			logger_log.warn(message);
			tx.commit();
			flag = true;
		} catch (Exception e) {
	...
	public boolean updateApiParameters(ApiParameters api) {
		...
		try {
			session.update(api);
			String message = "QUERY : Update Api || ID: " + api.getApiId();
			logger_log.warn(message);
			tx.commit();
			flag = true;
		} catch (Exception e) {
	...
	public List<ApiParameters> getApiParametersByNamedQuery(String query, Map<String, Object> param) {
		...
		try {
			...
			list = query_q.list();
			String message = "Method : getApiParametersByNamedQuery() ";
			logger_log.warn(message);
		} catch (Exception e) {
			String message = "Error Details :- " + e;
			logger_log.error(message);
